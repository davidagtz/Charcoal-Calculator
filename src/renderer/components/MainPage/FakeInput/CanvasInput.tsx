...
export default class CanvasInput extends Canvas<{
    ...
    render() {
        return (
            ...
            >
                ...
                />
                <canvas className="cin" id={this.props.id + '-canvas'} onMouseUp={this.focus} />
            </div>
        );
    }
        ...
        function draw(this: any, node: ParseNode, size: number = 1) {
            ...
            if (
                ...
                if (node.value === '+' || node.value === '-') {
                    ...
                    const pY = y;
                    y += (size * this.em) / 2;
                    if (!hasDrawn && index <= start && start <= index + 1) {
                        this.clearCursor();
                        this.cursor.x = x;
                        this.cursor.y = y;
            ...
            function putCursor(this: any, str: string) {
                ...
                if (!hasDrawn && index <= start && start <= index + length) {
                    this.clearCursor();
                    this.cursor.x = x + this.textWidth(str.substring(0, start - index));
                    this.cursor.y = y;
    ...
    drawCursor() {
        this.fill('#000');
        this.rect(this.cursor.x, this.cursor.y, 2, this.cursor.height * this.em);
    }
    clearCursor() {
        this.fill('#fff');
        this.rect(this.cursor.x, this.cursor.y, 2, this.cursor.height * this.em);
